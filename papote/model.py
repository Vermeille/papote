...
class RotarySingle(torch.nn.Module):
    ...
    def forward(self, q, positions=None, seq_dim=-2):
        seq_len = q.shape[seq_dim]
        device = q.device
        key = self._cache_key(device)
        cos_cached, sin_cached, cached_len = self._cache.get(key, (None, None, 0))
...
class NoRotary(nn.Module):
    ...
    def forward(self, q, k=None, v=None, positions=None):
        if k is None or v is None:
            # only q provided, no kv cache
            return q
        return q, k, v
...
class Transformer(nn.Module):
    def __init__(
        ...
        self.rotary = RotarySingle(hidden_size) if rotary_single else NoRotary()
